...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        for(int currRank = lastRankFilled - 1; currRank >= 0; currRank--) {
          if(filesWithQueens[currRank] == filesWithQueens[lastRankFilled]) return true;
          if(Math.abs(filesWithQueens[currRank] - filesWithQueens[lastRankFilled]) == Math.abs(currRank - lastRankFilled)) return true;
        }
        return false;
    }
    ...
    public boolean accept() {
        return ranks() - 1 == lastRankFilled && !lastIsNg();
    }
